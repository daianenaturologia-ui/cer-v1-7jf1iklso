...
class DemoAdapter {
   ...
   */
  /**
   * M2C: Migração local idempotente e não destrutiva para registros do Capítulo 1 no demoAdapter:
   * Registros `ayv_c1_*` sem `revision_number` passam a pertencer à revisão 1.
   * Não altera conteúdo, autoria, datas nem apaga nada.
   */
  public migrateLegacyChapter1ResponsesInStore(store: DemoStateStore): DemoStateStore {
    ...
    for (const r of store.experienceResponses) {
      ...
      if (sVal && typeof sVal === 'object') {
        ...
        sVal.metadata.chapter_id =
          sVal.metadata.chapter_id || 'capitulo-1-estrutura-caracteristicas'
  ...
  private loadState(): DemoStateStore {
    try {
      ...
      if (raw) {
        ...
        const migratedC1 = this.migrateLegacyChapter1ResponsesInStore(sanitizedAyv)
        const migratedC2 = this.migrateLegacyChapter2ResponsesInStore(migratedC1)
        return migratedC2
      }
    } catch (e) {
  ...
  public enableDemo(persona: 'mariana' | 'daiane' = 'mariana'): void {
    ...
    this.migrateLegacyChapter1ResponsesInStore(this.state)
