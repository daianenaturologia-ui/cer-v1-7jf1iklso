...
async function main() {
  ...
  const reportJsonPath = process.env.CER_REPORT_JSON_PATH || 'pav-report.json'
  try {
    ...
    const payload = {
      job_name: 'Primeiro Atendimento Verification (PAV-01..06)',
      ...
      summary: {
        ...
        all_pass: allPass && results.length === 6,
      },
      scenarios,
    }
  ...
  // Emissão da tabela individual PAV-01..PAV-06 no $GITHUB_STEP_SUMMARY se presente
  const summaryFile = process.env.GITHUB_STEP_SUMMARY
  if (summaryFile) {
    try {
      const fs = await import('node:fs')
      const summaryLines = []
      summaryLines.push('### 🩺 Verificação do Primeiro Atendimento (PAV-01..06)')
      summaryLines.push('')
      ...
      summaryLines.push('')
      summaryLines.push(
        `**Resultado Geral:** ${allPass && results.length === 6 ? '✅ SUCESSO COMPLETO' : '❌ FALHA / BLOQUEIO'} (${passCount} PASS, ${failCount} FAIL, ${blockedCount} BLOCKED)`,
      )
      summaryLines.push('')
